...
void geometric_adjust(double& a, double& b, double& L, double& h, double theta, double L_v){
    L = L_v / cos(theta);
    b = L * tan(psi);
...
void batch_calc(double theta){
    ...
    for(double L_v = 10.0e5; L_v <= 15.0e5; L_v += 50000){
        
        double h, k = 0.0;
        double a, b, L;
        geometric_adjust(a, b, L, h, theta, L_v);
            ...
            for(double j = k - b; j < k + b; j+=spacing){
                ...
                if(pow((i-h) / a, 2) + pow((j-k) / b, 2)<=1.0){
                    ...
                    for(int cnt = 0; cnt<5; cnt++){
                        ...
                        if(eta < 0.1) continue;
                        else probability[cnt] += 1.0e9 * cos_delta * exp(-1.0 * t_total / tau) / pow(dist, 2.0); //deriving survival probability
                    }
                }
            }
        }
