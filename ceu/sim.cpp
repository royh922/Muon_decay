    ...
    for(double i = h - a; i < h + a; i+=spacing){
        for(double j = k - b; j < k + b; j+=spacing){
            ...
            if(pow((i-h) / a, 2) + pow((j-k) / b, 2)<=1.0){
                ...
                probability += cos_theta * cos_delta * exp(-1.0 * t_total / tau) / pow(dist, 2.0); //deriving survival probability
...
int main(){
    ...
    for(int i = 0; i<n; i++){
        ...
        batch_calc(theta);
        cout<<i<<" "<<theta / pi * 180<<" "<<batch_prob<<"\n";
        output<<theta / pi * 180<<" "<<batch_prob<<"\n";
    }
    output.close();
}