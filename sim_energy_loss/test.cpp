...
void energy_adjust(double E_temp){
    ...
    /* for(double i = h - a; i < h + a; i+=spacing){
        for(double j = k - b; j < k + b; j+=spacing){
            if(pow((i-h) / a, 2) + pow((j-k) / b, 2)<=1.0){
                ...
                t = Gamma * tau;
                probability = 1.0e40 * scope_area / (pow(dist,2) * 2 * pi) * exp(-1.0 * t_total / t); //deriving survival probability
                if(montecarlo(probability)) not_decayed++;
            }
        }
    } */
    batch_prob = (double) not_decayed / (a * b);
